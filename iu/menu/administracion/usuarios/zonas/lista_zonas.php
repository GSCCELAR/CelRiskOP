<?php
	...
	if ($busca_usuario != "")
		$add_query .= " AND uz.usuario_id = '" . Seguridad::escapeSQL($busca_usuario) . "'";
	...
	*/
	...
	$count = 0;
	$add_query .= " ";
	$result = BD::sql_query("select COUNT(*) AS count FROM usuario_zona uz WHERE 1=1 $add_query");
	...
	$query = "select uz.id, l.nombre 'zona'
		FROM lista l
		JOIN usuario_zona uz ON l.id = uz.zona_id
		WHERE 1=1 $add_query
		GROUP BY l.id
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$btnBorrar = "<img style='margin:3px;cursor:pointer;'  onclick='eliminarZona(" . $row["id"] . ")' src='imagenes/eliminar.png'>";
		$responce->rows[$i]['cell'] = array (
			utf8_encode(htmlentities($row["zona"], ENT_QUOTES,'iso8859-1')),
