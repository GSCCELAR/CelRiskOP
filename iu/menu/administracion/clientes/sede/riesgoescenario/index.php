...
<script type="text/javascript" charset="ISO-8859-1">
	...
	$(document).ready(function() {
		$("#lista_escenario").jqGrid({
		    ...
		    datatype : "json",
		    colNames : ['<b>Nombre del RIESGO</b>', '<b>ESCENARIOS</b>', '<b>Opciones</b>'],
		    colModel : [
			...
			subGridRowExpanded: function(subgrid_id, row_id) {
				...
				$("#"+subgrid_table_id).jqGrid({
					url:"<?php echo DIR_WEB; ?>lista_riesgos.php?sede_escenario_id=" + row_id+"&sedeid="+<?php echo $id; ?>, 
					datatype: "json",
					colNames: ['Escenarios en los que se presenta','Opc'], 
					colModel: [
	...
	function addEscenario() {
		var idriesgo = $("#escenario").val();
		if(idriesgo != "")
		{
			showMessage();
			$.ajax({
				//url: '<?php echo DIR_WEB ;?>set_escenarios.php',
				url: '<?php echo DIR_WEB ;?>set_riesgos.php',
				data: {escenario_id : idriesgo, sede_id: "<?php echo $id; ?>"}
			}).done(function(data)
			{			
				...
				if (/^ok$/.test(data)) {
					...
					//mensaje("El nuevo RIESGO <br />Ha sido agregado", "error");
				} else {
					//mensaje("Error", data, "error");
					mensaje("Error", "No fue posible adicionar el 	RIESGO <br />Es posible que el RIESGO ya se encuentre asignado a la sede", "error");
				}
	...
	//Nunca se sabra lo que siente un sapo cuando se sienta en el fondo del estanque e interpreta el mundo de sapo que le rodea
