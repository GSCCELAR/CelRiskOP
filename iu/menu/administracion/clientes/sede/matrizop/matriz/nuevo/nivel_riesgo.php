<?php
    ...
    if($row = BD::obtenerRegistro($result)) {
        ...
        $html = "<div style='background-color:".$row["color"].";padding:5px;height:100%;width:210px;text-align:center;'><span style='color:$color;font-weight:bold;'>".nivelesRiesgo($row["calificacion"])."</span></div>";
    }
    echo $html;



    function nivelesRiesgo($riesgo)
    {
        if($riesgo == "BAJO")
            return "ACEPTABLE";
        else if($riesgo == "MODERADO")
            return "TOLERABLE";
        else if($riesgo == "IMPORTANTE")
            return "MODERADO";
        else if($riesgo == "CRÍTICO")
            return "IMPORTANTE";
    }
?>