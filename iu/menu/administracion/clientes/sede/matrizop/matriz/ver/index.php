<?php
	...
	if (count($items) > 0) {
		$result_query = BD::sql_query("SELECT r.id, r.numero_item, c.mprobabilidad_id, c.mseveridad_id, c.mnivel_riesgo_id, r.mriesgo_id, r.fecha_inicio, fecha_fin 
			FROM mriesgo_op r, mclasificacion_riesgo_operativo c 
	...
	$query = BD::sql_query("SELECT CASE 
								WHEN (id = 5) THEN \"INSIGNIFICANTE\" 
								WHEN (id = 1) THEN \"MENOR\"
								WHEN (id = 2) THEN \"MODERADO\"
								WHEN (id = 3) THEN \"MAYOR\"
								WHEN (id = 4) THEN \"CATASTROFICO\"
							END AS calificacion, criterio, orden, id
							FROM mseveridad 
							ORDER BY orden ASC");
	...
	$query = BD::sql_query("SELECT CASE 
								WHEN (id = 5) THEN \"RARO\" 
								WHEN (id = 1) THEN \"IMPROBABLE\"
								WHEN (id = 2) THEN \"POSIBLE\"
								WHEN (id = 3) THEN \"PROBABLE\"
								WHEN (id = 4) THEN \"CASI CERTERO\"
							END AS calificacion, criterio, orden, id
							FROM mprobabilidad 
							ORDER BY orden DESC");
	...
	$query = BD::sql_query("SELECT * FROM mnivel_riesgo_operativo");
	while($row = BD::obtenerRegistro($query))
	{
	...
	$query = BD::sql_query("SELECT * FROM mclasificacion_riesgo_operativo");
	while($row = BD::obtenerRegistro($query))
	{
...
<table cellpadding=3 width="100%" border=1>
    <tr class="titulo">
        <td colspan="2" ></td>
        <td colspan="<?php  echo count($severidad) ;?>">IMPACTO</td>
    </tr>
    <tr class="titulo">
        <td colspan="2" ></td>
        <?php 
    ...
    <tr>
        ...
        foreach ($probabilidad as $pk => $vp) {
			echo "<tr>";
			if (!$titulo) {
				echo "<td class='titulo-vertical' rowspan=" . count($probabilidad) . " >PROB OCURRENCIA</td>";
				$titulo = true;
			}
