<?php
	...
	if (isset($_POST["mpeligro_id"]) && isset($_POST["mprobabilidad_id"]) && isset($_POST["mseveridad_id"])) {
		...
		$sql = BD::sql_query("SELECT IFNULL(MAX(numero_item) + 1,1) AS numero_item FROM mriesgo WHERE sector_id = ". $_POST["sector_id"]. " AND dispositivo_seguridad_id IS NULL");
		if($f = BD::obtenerRegistro($sql))
...
<form method="post" name="formNuevoMatrizBase" id="formNuevoMatrizBase" action="<?php echo DIR_WEB . basename(__FILE__); ?>">
	...
	<div  id="seccion-1" style="height:350px; overflow: auto;" class="tabs">
		<table cellpadding=3 border=0 width="100%">
			<tr>
				<td align=right><b style="font-weight:bold;">Sector:</b></td>
				<td>
					<select style='padding:1px; height:25px; width:100%;' name='sector_id' id='sector_id'>
					<option value="">Seleccione...</option>
						<?php
							$lista = new Lista();
							$lista->writeOptions(-1, array("id", "nombre"),array("tipo_lista_codigo" => "SECTOR")," ORDER BY nombre");
						?>
					</select>
				</td>	
			</tr>
			...
			<tr>
				<td align=right valign=top><b style="font-weight:bold;">Peligro:</b></td>
				<td valign=top>
			...
			<tr class="peligro_view" style="visibility:hidden;">
				...
				<td valign=top> 
					<select style='padding:1px; height:25px; width:220px;' name='peligros_riesgo' id='peligros_riesgo'>
					</select>
				</td>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#ventana2").dialog({
			...
			title: "<i class='icon-white icon-plus-sign' /> &nbsp; Nueva matriz",
			resizable: false,
			width: 800,
			open : function() {
		...
		$("#formNuevoMatrizBase").validate({
			...
			submitHandler: function(e) {
				showMessage();
				$('#formNuevoMatrizBase').ajaxSubmit({
		...
		$("#formNuevoMatrizBase").css("font-size", "14px");
