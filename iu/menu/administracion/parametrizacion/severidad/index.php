...
?>
<div><button onclick="nuevaSeveridad();" class='btn btn-success'><i class='icon icon-plus icon-white' /> Nuevo</button></div>
...
<script type="text/javascript" charset="ISO-8859-1">
	$(document).ready(function() {
		$("#lista").jqGrid({
		    ...
		    datatype : "json",
		    colNames : ['<b>ID</b>', '<b>Calificación</b>','<b>Criterio</b>', '<b>Opc</b>'],
		    colModel : [
		        { name:'id', index:'id', width:65, align: 'center' },
		        { name:'calificacion', index:'calificacion', width:150, align: 'left' },
				{ name:'criterio', index:'criterio', width:500, align: 'left' },
		    ...
		    sortorder : "asc",
			height: "auto",
		    caption: "<i class='icon-list icon-white' /> <b>Severidad</b>"
		});
	...
	function editarProbabilidad(id) {
		showMessage();
		$("#ventana3").load("<?php echo DIR_WEB; ?>editar.php", { severidad_id: id }, function () {
			hideMessage();
		});
	}

	function nuevaSeveridad() {
		showMessage();
	...
	function eliminarProbabilidad(id) {
		Swal.fire({
					title : '',
					text : '¿Confirma que desea eliminar la Severidad?',
					type : 'warning',
					...
					if (res.value)
					{
						...
						$.post("<?php echo DIR_WEB; ?>index.php", { delete : id }, function(res) {
							hideMessage();
							if (!/^ok$/.test(res))
								mensaje("Error", "No fue posible borrar la Severidad<br />", "error");
