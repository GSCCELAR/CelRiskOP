<?php
	...
	*/
	$campos = array("id", "nombre", "objetivo", "alcance");
	...
	$count = 0;
	$add_query .= " ";
	$result = BD::sql_query("select COUNT(*) AS count FROM proceso WHERE 1=1 $add_query");
	...
	$query = "select * FROM proceso WHERE 1=1 $add_query ORDER BY $sidx $sord";
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$responce->rows[$i]['cell'] = array (
			...
			utf8_encode(htmlentities($row["objetivo"], ENT_QUOTES, "iso8859-1")),
			utf8_encode(htmlentities($row["alcance"], ENT_QUOTES, "iso8859-1")),
