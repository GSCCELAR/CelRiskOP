<?php
	...
	*/
	$campos = array("id", "nombre", "descripcion", "costo_anual", "porcentaje_eficiencia");
	...
	$count = 0;
	$add_query .= " ";
	$result = BD::sql_query("select COUNT(*) AS count FROM control WHERE 1=1 $add_query");
	...
	$query = "select * FROM control WHERE 1=1 $add_query ORDER BY $sidx $sord";
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$responce->rows[$i]['cell'] = array (
			...
			utf8_encode(htmlentities($row["costo_anual"], ENT_QUOTES, "iso8859-1")),
			utf8_encode(htmlentities($row["porcentaje_eficiencia"], ENT_QUOTES, "iso8859-1")),
