<?php
	...
	*/
	$campos = array("id", "nombre", "descripcion", "cantidad");
	...
	$count = 0;
	$add_query .= " ";
	$result = BD::sql_query("select COUNT(*) AS count FROM vtipo_lista WHERE 1=1 $add_query");
	...
	$query = "select * FROM vtipo_lista WHERE 1=1 $add_query ORDER BY $sidx $sord";
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$responce->rows[$i]['cell'] = array (
			//utf8_encode($row["id"]),
			utf8_encode(htmlentities($row["codigo"], ENT_QUOTES, "iso8859-1")),
			utf8_encode(htmlentities($row["descripcion"], ENT_QUOTES, "iso8859-1")),
			utf8_encode($row["cantidad"]),
			utf8_encode("<button class='btn btn-small btn-success' onclick='nuevo(" . $row["id"] . ")'><i class='icon icon-white icon-plus'></i></button>")
		);
		$i++;
	}
