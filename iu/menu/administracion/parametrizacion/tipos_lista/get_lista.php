<?php
	...
	$tipo_lista_id = isset($_GET["tipo_lista_id"]) ? intval($_GET["tipo_lista_id"]) : "";
	...
	*/
	...
	if (!$sidx)
		$sidx = 1;
	$count = 0;
	$result = BD::sql_query("select COUNT(*) AS count FROM lista WHERE tipo_lista_id=$tipo_lista_id");
	...
	if ($start < 0)
		$start = 0;
	$query = "select * FROM lista WHERE tipo_lista_id=$tipo_lista_id ORDER BY $sidx $sord";
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$botones = "<img style='margin:3px;cursor:pointer;' onclick='editar(" . $row["id"] . ")' src='imagenes/editar.png'>"
			. "<img style='margin:3px;cursor:pointer;' onclick='eliminar(" . $row["tipo_lista_id"] . "," . $row["id"] . ")' src='imagenes/eliminar.png'>";
