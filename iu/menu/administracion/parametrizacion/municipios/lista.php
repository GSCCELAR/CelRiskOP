<?php
	...
	*/
	$campos = array("id", "nombre", "pais_nombre", "descripcion");
	...
	$count = 0;
	$add_query .= " ";
	$result = BD::sql_query("select COUNT(*) AS count FROM vmunicipios WHERE 1=1 $add_query");
	...
	$query = "select * FROM vmunicipios WHERE 1=1 $add_query ORDER BY $sidx $sord";
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$responce->rows[$i]['cell'] = array (
			utf8_encode($row["id"]),
			utf8_encode(htmlentities($row["pais_nombre"], ENT_QUOTES, "iso8859-1")),
			utf8_encode(htmlentities($row["departamento_nombre"], ENT_QUOTES, "iso8859-1")),
