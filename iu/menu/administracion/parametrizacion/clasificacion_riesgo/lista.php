<?php
	...
	*/
	$campos = array("id", "probabilidad","severidad","nivel_riesgo");
	...
	$count = 0;
	$add_query .= " ";
	$result = BD::sql_query("SELECT COUNT(*) AS count FROM vmclasificacion_riesgo WHERE 1=1 $add_query");
	...
	$query = "SELECT * FROM vmclasificacion_riesgo WHERE 1=1 $add_query ORDER BY $sidx $sord";
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$responce->rows[$i]['cell'] = array (
			...
			utf8_encode(htmlentities($row["nivel_riesgo"],ENT_QUOTES, "iso8859-1")),
			utf8_encode("<img style='margin:3px;cursor:pointer;' onclick='editarClasificacionRiesgo(" . $row["id"] . ")' src='imagenes/editar.png'>"
				. "<img style='margin:3px;cursor:pointer;' onclick='eliminarClasificacionRiesgo(" . $row["id"] . ")' src='imagenes/eliminar.png'>")
		);
		$i++;
	}
