<?php
	...
	if (isset($_POST["delete"])) {
		$reg = new Probabilidad();
		$reg->load($_POST["delete"]) or die("Error al cargar la información");
		die ($reg->delete() ? "ok" : "Hay datos relacionados con este item"); 
	}
?>
<div><button onclick="nuevaProbabilidad();" class='btn btn-success'><i class='icon icon-plus icon-white' /> Nuevo</button></div>
...
<script type="text/javascript" charset="ISO-8859-1">
	$(document).ready(function() {
		$("#lista").jqGrid({
		    ...
		    datatype : "json",
		    colNames : ['<b>ID</b>', '<b>Calificación</b>','<b>Criterio</b>','<b>Descripción</b>', '<b>Opc</b>'],
		    colModel : [
		        { name:'id', index:'id', width:65, align: 'center' },
		        { name:'calificacion', index:'calificacion', width:150, align: 'left' },
				{ name:'criterio', index:'criterio', width:260, align: 'left' },
				{ name:'descripcion', index:'descripcion', width:260, align: 'left' },
		    ...
		    sortorder : "asc",
			height: "auto",
		    caption: "<i class='icon-list icon-white' /> <b>Probabilidad</b>"
		});
	...
	function editarProbabilidad(id) {
		showMessage();
		$("#ventana3").load("<?php echo DIR_WEB; ?>editar.php", { probabilidad_id: id }, function () {
			hideMessage();
		});
	}

	function nuevaProbabilidad() {
		showMessage();
	...
	function eliminarProbabilidad(id) {
		Swal.fire({
					title : '',
					text : '¿Confirma que desea eliminar la Probabilidad?',
					type : 'warning',
					...
					if (res.value)
					{
						...
						$.post("<?php echo DIR_WEB; ?>index.php", { delete : id }, function(res) {
							hideMessage();
							if (!/^ok$/.test(res))
								mensaje("Error", "No fue posible borrar la Probabilidad<br />", "error");
