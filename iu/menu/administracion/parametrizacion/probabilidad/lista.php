<?php
	...
	*/
	$campos = array("id", "calificacion","criterio","descripcion");
	...
	$count = 0;
	$add_query .= " ";
	$result = BD::sql_query("SELECT COUNT(*) AS count FROM mprobabilidad WHERE 1=1 $add_query");
	...
	$query = "SELECT * FROM mprobabilidad WHERE 1=1 $add_query ORDER BY $sidx $sord";
	...
	while ($row = BD::obtenerRegistro($result)) {
		...
		$responce->rows[$i]['cell'] = array (
			...
			utf8_encode("<img style='margin:3px;cursor:pointer;' onclick='editarProbabilidad(" . $row["id"] . ")' src='imagenes/editar.png'>"
				. "<img style='margin:3px;cursor:pointer;' onclick='eliminarProbabilidad(" . $row["id"] . ")' src='imagenes/eliminar.png'>")
		);
		$i++;
	}
